...
namespace eCommerceApp.Application.Services.Implementations.Authentication
{
    public class AuthenticationService
        ...
        IMapper mapper, IValidator<CreateUser> createUserValidator, IValidator<LoginUser> loginUserValidator, IValidationService validationService) : IAuthenticationService
    {
        public async Task<ServiceResponse> CreateUser(CreateUser user)
        {
            ...
            if (!assignedResult)
            {
                ...
                if (removeUserResult <= 0)
                {
                    ...
                    logger.LogError
                        (new Exception($"User with Email {_user.Email} failed to be removed as a result of role assignin issue"),
                        "User could not be assigned to Role");
                    return new ServiceResponse { Message = "Error occurred while creating account" };
                }
            }
            return new ServiceResponse { Success = true, Message = "Account Successfully created!" };
            // TODO verify email
        }
        public async Task<LoginResponse> LoginUser(LoginUser user)
        {
            ...
            if (!loginResult)
                return new LoginResponse(Message: "Email not found or invalid crederntials");
            var _user = await userManagement.GetUserByEmail(user.Email);
